...
{
    public partial class Havale_Yap_Ticari : Form
    {
        public Havale_Yap_Ticari()
        {
            InitializeComponent();
        }
        ...
        private void btnHavaleYap_Click(object sender, EventArgs e)
        {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var cmd = new NpgsqlCommand("SELECT bakiye FROM ticarimusteri WHERE hesapno = @recipientAccountNumber", conn))
                {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var cmd = new NpgsqlCommand("SELECT bakiye FROM ticarimusteri WHERE hesapno = @senderAccountNumber", conn))
                {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var cmd = new NpgsqlCommand("UPDATE ticarimusteri SET bakiye = @recipientBalance WHERE hesapno = @recipientAccountNumber", conn))
                {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var cmd = new NpgsqlCommand("UPDATE ticarimusteri SET bakiye = @senderBalance WHERE hesapno = @senderAccountNumber", conn))
                {
